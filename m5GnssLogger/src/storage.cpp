...
bool StorageModule::writeData(const GNSS_DATA &data, const char *fileName) {
  ...
  char lineStr[128];
  sprintf(lineStr, "%s,%lf,%lf,%.1f,%.1f,%d,%.2f", 
          datetime, data.lat, data.lng, data.alt, 
          data.vel, data.siv, data.hdop);
...
void StorageModule::generateFileName(const char *baseName, char *buffer, 
                                    size_t bufferSize, const GNSS_DATA &data, 
                                    bool isRaw) {
  if (isRaw) {
    snprintf(buffer, bufferSize, "/%s_%04d%02d%02d_%02d%02d%02d_raw.csv", 
             baseName, data.year, data.month, data.day, 
             data.hour, data.minute, data.second);
  } else {
    snprintf(buffer, bufferSize, "/%s_%04d%02d%02d_%02d%02d%02d.csv", 
             baseName, data.year, data.month, data.day, 
             data.hour, data.minute, data.second);
  }
}
